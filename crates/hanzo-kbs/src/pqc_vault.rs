...
use crate::{
    ...
    types::{AttestationType, KeyId, PrivacyTier},
    vault::KeyVault,
};
...
use hanzo_pqc::{
    ...
    signature::{MlDsa, Signature, SignatureAlgorithm, SigningKey, VerifyingKey},
};
...
impl KeyVault for PqcVault {
    ...
    async fn store_key(&self, key_id: &KeyId, key_data: &[u8]) -> Result<()> {
        ...
        let output = self.ml_kem.encapsulate(&encap_key).await.map_err(|e| {
            SecurityError::CryptoError(format!("Key encapsulation failed: {:?}", e))
        })?;
        ...
        let wrapping_key = kdf
            .derive(None, &output.shared_secret, b"hanzo-vault-wrap-v1", 32)
        ...
        R: Send,
    {
        ...
        if wrapped.len() < 4 {
            return Err(SecurityError::CryptoError(
                "Invalid wrapped key format".into(),
            ));
        }
        ...
        let shared_secret = self
            .ml_kem
            .decapsulate(&protected.decap_key, kem_ct)
            .await
        ...
        let kdf = HkdfKdf::new(self.config.kdf);
        let wrapping_key = kdf
            .derive(None, &shared_secret, b"hanzo-vault-wrap-v1", 32)
            .map_err(|e| SecurityError::CryptoError(format!("KDF failed: {:?}", e)))?;
