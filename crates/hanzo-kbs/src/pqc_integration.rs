...
use hanzo_pqc::{
    config::PqcConfig,
    hybrid::{HybridEncapsulationKey, HybridKem, HybridMode},
    kem::{DecapsulationKey, EncapsulationKey, Kem, KemAlgorithm, MlKem},
    privacy_tiers::PrivacyTier as PqcPrivacyTier,
    signature::{DigitalSignature, MlDsa, Signature, SignatureAlgorithm, SigningKey, VerifyingKey},
};
...
impl PqcKbs {
    ...
    pub async fn generate_kem_keypair(&self) -> Result<(EncapsulationKey, DecapsulationKey)> {
        ...
        let keypair = self.ml_kem.generate_keypair(alg).await.map_err(|e| {
            SecurityError::CryptoError(format!("KEM keypair generation failed: {:?}", e))
        })?;
    ...
    pub async fn generate_signing_keypair(&self) -> Result<(VerifyingKey, SigningKey)> {
        ...
        self.ml_dsa.generate_keypair(alg).await.map_err(|e| {
            SecurityError::CryptoError(format!("Signing keypair generation failed: {:?}", e))
        })
    }
    ...
    pub async fn wrap_dek_with_mlkem(
        ...
        let output =
            self.ml_kem.encapsulate(encap_key).await.map_err(|e| {
                SecurityError::CryptoError(format!("Encapsulation failed: {:?}", e))
            })?;

        // Use shared secret to derive wrapping key
        use hanzo_pqc::kdf::{HkdfKdf, Kdf};
        let kdf = HkdfKdf::new(self.config.kdf);
        let wrapping_key = kdf
            .derive(None, &output.shared_secret, b"hanzo-kbs-dek-wrap-v1", 32)
    ...
    pub async fn unwrap_dek_with_mlkem(
        ...
        if wrapped_dek.len() < 4 {
            return Err(SecurityError::CryptoError(
                "Invalid wrapped DEK format".into(),
            ));
        }
        ...
        let kdf = HkdfKdf::new(self.config.kdf);
        let wrapping_key = kdf
            .derive(None, &shared_secret, b"hanzo-kbs-dek-wrap-v1", 32)
            .map_err(|e| SecurityError::CryptoError(format!("KDF failed: {:?}", e)))?;
    ...
    pub async fn verify_attestation(
        ...
        self.ml_dsa
            .verify(verifying_key, report, signature)
            .await
            .map_err(|e| {
                SecurityError::CryptoError(format!("Attestation verification failed: {:?}", e))
            })
    }
}
...
impl PqcKbs {
    ...
    pub async fn initiate_handshake(
        &self,
        hybrid: bool,
    ) -> Result<(PqcHandshake, DecapsulationKey)> {
        if hybrid && self.config.hybrid {
            // Use hybrid mode
            let hybrid_kem = HybridKem::new(HybridMode::default());
            let (encap_key, _decap_key) = hybrid_kem
                .generate_keypair(HybridMode::default())
                .await
                .map_err(|e| {
                    SecurityError::CryptoError(format!("Hybrid keypair generation failed: {:?}", e))
                })?;
...
mod tests {
    ...
    async fn test_dek_wrapping() {
        ...
        let unwrapped = kbs
            .unwrap_dek_with_mlkem(&wrapped, &decap_key)
            .await
            .unwrap();
    ...
    async fn test_attestation_signing() {
        ...
        let valid = kbs
            .verify_attestation(report, &signature, &verifying_key)
            .await
            .unwrap();
