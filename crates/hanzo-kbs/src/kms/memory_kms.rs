...
use crate::types::{AgentDek, KeyAuditEntry, KeyId, KeyOperation, RootKey, TenantKek};
...
impl KeyManagementService for MemoryKms {
    ...
    async fn create_tenant_kek(&self, tenant_id: &str) -> Result<TenantKek> {
        ...
        let kek_material =
            crate::kms::KeyHierarchy::derive_tenant_kek(&self.root_key_material, tenant_id);
        ...
        // Store metadata
        self.tenant_keks
            .insert(tenant_id.to_string(), (kek.clone(), kek_material));

        self.key_info.insert(
            kek_id.clone(),
            KeyInfo {
                key_id: kek_id.clone(),
                key_type: KeyType::TenantKek {
                    tenant_id: tenant_id.to_string(),
                },
    ...
    async fn create_agent_dek(&self, agent_id: &str, tenant_id: &str) -> Result<AgentDek> {
        // Get tenant KEK
        let (_, tenant_kek_material) = self
            .tenant_keks
            .get(tenant_id)
            .ok_or_else(|| {
                SecurityError::KeyNotFound(format!("Tenant KEK not found: {}", tenant_id))
            })?
            .clone();
        ...
        // Store metadata
        self.agent_deks
            .insert(agent_id.to_string(), (dek.clone(), dek_material));
    ...
    async fn get_audit_logs(
        ...
        let mut logs: Vec<KeyAuditEntry> = self
            .audit_log
            .iter()
        ...
        if let Some(f) = filter {
            logs.retain(|log| {
                ...
                if let Some(ref ops) = f.operations {
                    keep &= ops.iter().any(|op| {
                        std::mem::discriminant(op) == std::mem::discriminant(&log.operation)
                    });
                }
    ...
    async fn import_customer_key(
        ...
        tenant_id: &str,
        _wrapped_key: &[u8],
        _key_metadata: CustomerKeyMetadata,
    ) -> Result<KeyId> {
        ...
        Err(SecurityError::Other(anyhow::anyhow!(
            "Customer key import not implemented"
        )))
    }
    ...
    async fn register_customer_held_key(
        ...
        Err(SecurityError::Other(anyhow::anyhow!(
            "HYOK registration not implemented"
        )))
    }
}
