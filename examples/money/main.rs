...
//!
//! Operations: GET /v1/billing/balance (get_v1_billing_balance) and
//! GET /v1/billing/usage (get_v1_billing_usage).
//!
//! Both declare only a `default` response with no content schema in the document,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    billing_api::get_v1_billing_balance(&cfg).await?;
    let balance = get_json(&cfg, "/v1/billing/balance").await?;
    println!("balance  {balance}");

    billing_api::get_v1_billing_usage(&cfg).await?;
